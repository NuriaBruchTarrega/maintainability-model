...
tuple[int, Rank] calculateDuplicationMetric(Files files, int volume) {
	set[loc] duplicates = findDuplicates(files);
	int duplicationPercentage = percent(size(duplicates), volume);
